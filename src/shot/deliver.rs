    ...
    match destination.explicit.as_deref() {
        ...
        None => {
            if let Some(directory) = &destination.directory {
                let stem =
                    expand_file_name(&destination.file_name, &glib::DateTime::now_local().ok());
                let path = unique_path(directory, &stem, |path| path.exists());
...
pub fn expand_file_name(template: &str, now: &Option<glib::DateTime>) -> String {
    let expanded = now
        .as_ref()
...
/// `directory/stem.png`, or `stem-2.png`, `stem-3.png`… if taken. Two shots in
/// the same second must not overwrite each other.
pub fn unique_path(directory: &Path, stem: &str, exists: impl Fn(&Path) -> bool) -> PathBuf {
    let first = directory.join(format!("{stem}.png"));
    if !exists(&first) {
        return first;
    }
    (2..)
...
fn copy_with_wl_copy(png: &[u8]) -> Result<(), String> {
    ...
    child
        ...
        .write_all(png)
        .map_err(|error| format!("cannot send the image to wl-copy: {error}"))?;
...
mod tests {
    ...
    fn the_template_is_expanded_from_the_clock() {
        ...
        assert_eq!(
            expand_file_name("Screenshot_%Y-%m-%d_%H-%M-%S", &moment),
    ...
    fn a_template_cannot_escape_the_directory_or_double_the_extension() {
        let moment = glib::DateTime::from_local(2026, 9, 16, 14, 5, 9.0).ok();
        assert_eq!(expand_file_name("%Y/%m.png", &moment), "2026-09");
        // Leading dots are dropped too, so a name cannot make a hidden file.
        assert_eq!(expand_file_name("../shot", &None), "-shot");
        assert_eq!(expand_file_name("   ", &None), "Screenshot");
    }
    ...
    fn a_taken_name_gets_a_counter() {
        ...
        assert_eq!(
            unique_path(directory, "b", |path| taken.contains(path)),
            directory.join("b.png")
        );
        assert_eq!(
            unique_path(directory, "a", |path| taken.contains(path)),
